export const dynamic = "force-dynamic";
export const revalidate = 0;

import Link from "next/link";
import { createClient } from "@supabase/supabase-js";

type Customer = {
  id: string;
  company_name: string | null;
  address_line_1: string | null;
  address_line_2: string | null;
  city: string | null;
  state: string | null;
  country: string | null;
  postal_code: string | null;
  contact_person: string | null;
  email: string | null;
  phone: string | null;
  website: string | null;
  gst_number: string | null;
  pan_number: string | null;
  iec_number: string | null;
  notes: string | null;
};

type EditCustomerPageProps = {
  params: Promise<{
    id: string;
  }>;
};

function value(customer: Customer | null, key: keyof Customer) {
  return customer?.[key] ? String(customer[key]) : "";
}

export default async function EditCustomerPage({
  params,
}: EditCustomerPageProps) {
  const { id } = await params;

  const supabase = createClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.SUPABASE_SERVICE_ROLE_KEY!
  );

  const { data, error } = await supabase
    .from("company_profiles")
    .select(
      "id, company_name, address_line_1, address_line_2, city, state, country, postal_code, contact_person, email, phone, website, gst_number, pan_number, iec_number, notes"
    )
    .eq("id", id)
    .maybeSingle();

  const customer = (data || null) as Customer | null;

  if (error || !customer) {
    return (
      <div className="p-10 space-y-6">
        <h1 className="text-3xl font-bold">Edit Customer</h1>

        <div className="p-4 rounded-lg bg-red-50 text-red-700">
          {error?.message || "Customer not found"}
        </div>

        <Link href="/customers" className="px-4 py-2 border rounded-lg">
          Back to Customers
        </Link>
      </div>
    );
  }

  return (
    <div className="p-10 space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Edit Customer</h1>
          <p className="text-sm text-gray-500 mt-1">
            Update customer profile and communication details.
          </p>
        </div>

        <div className="flex gap-3">
          <Link
            href={`/customers/${customer.id}/dashboard`}
            className="px-4 py-2 rounded-lg bg-purple-50 text-purple-700 border border-purple-200 hover:bg-purple-100"
          >
            Dashboard
          </Link>

          <Link
            href={`/customers/${customer.id}/timeline`}
            className="px-4 py-2 rounded-lg bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100"
          >
            Timeline
          </Link>

          <Link
            href={`/customers/${customer.id}/email`}
            className="px-4 py-2 rounded-lg bg-gray-100 border hover:bg-gray-200"
          >
            Send Email
          </Link>

          <Link href="/customers" className="px-4 py-2 border rounded-lg">
            Back to Customers
          </Link>
        </div>
      </div>

      <form action="/api/customers/save" method="POST" className="space-y-8">
        <input type="hidden" name="id" value={customer.id} />

        <div className="bg-white border rounded-xl p-6 space-y-4">
          <h2 className="text-xl font-semibold">Customer Details</h2>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1">
                Customer Company Name
              </label>
              <input
                name="company_name"
                defaultValue={value(customer, "company_name")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
                required
              />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">
                Buyer Contact Person
              </label>
              <input
                name="contact_person"
                defaultValue={value(customer, "contact_person")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
              />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">
                Buyer Email
              </label>
              <input
                name="email"
                type="email"
                defaultValue={value(customer, "email")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
              />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">
                Buyer Phone
              </label>
              <input
                name="phone"
                defaultValue={value(customer, "phone")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
              />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">
                Customer Website
              </label>
              <input
                name="website"
                defaultValue={value(customer, "website")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
              />
            </div>
          </div>
        </div>

        <div className="bg-white border rounded-xl p-6 space-y-4">
          <h2 className="text-xl font-semibold">Buyer Address</h2>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium mb-1">
                Address Line 1
              </label>
              <input
                name="address_line_1"
                defaultValue={value(customer, "address_line_1")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
              />
            </div>

            <div className="md:col-span-2">
              <label className="block text-sm font-medium mb-1">
                Address Line 2
              </label>
              <input
                name="address_line_2"
                defaultValue={value(customer, "address_line_2")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
              />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">City</label>
              <input
                name="city"
                defaultValue={value(customer, "city")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
              />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">State</label>
              <input
                name="state"
                defaultValue={value(customer, "state")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
              />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">Country</label>
              <input
                name="country"
                defaultValue={value(customer, "country")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
              />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">
                Postal Code
              </label>
              <input
                name="postal_code"
                defaultValue={value(customer, "postal_code")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
              />
            </div>
          </div>
        </div>

        <div className="bg-white border rounded-xl p-6 space-y-4">
          <h2 className="text-xl font-semibold">Tax / Import Details</h2>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1">
                GST / VAT / Tax Number
              </label>
              <input
                name="gst_number"
                defaultValue={value(customer, "gst_number")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
              />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">
                PAN / Company Tax ID
              </label>
              <input
                name="pan_number"
                defaultValue={value(customer, "pan_number")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
              />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1">
                IEC / Importer Code
              </label>
              <input
                name="iec_number"
                defaultValue={value(customer, "iec_number")}
                className="w-full border rounded-lg px-4 py-3 text-sm"
              />
            </div>
          </div>
        </div>

        <div className="bg-white border rounded-xl p-6 space-y-4">
          <h2 className="text-xl font-semibold">Notes</h2>

          <textarea
            name="notes"
            defaultValue={value(customer, "notes")}
            placeholder="Add buyer preferences, special instructions, shipping preferences, documentation requirements, etc."
            className="w-full border rounded-lg px-4 py-3 text-sm min-h-32"
          />
        </div>

        <button className="px-6 py-3 rounded-lg bg-gray-900 text-white text-sm">
          Save Customer
        </button>
      </form>
    </div>
  );
}