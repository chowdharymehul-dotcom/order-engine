...
function getGroupKey(item: OrderItem) {
  /*
    Important:
    Do NOT group only by message ID.

    Some old imported rows share message/thread IDs and then many unrelated
    SKUs get mixed into one huge row.

    This keeps "one row per real email/order" while preventing unrelated
    orders from being merged.
  */
  return [
    getMessageKey(item),
...
export default async function OrdersPage({ searchParams }: OrdersPageProps) {
  ...
  /*
    Pull all email rows needed for lookup.

    This is intentionally broad because older order_items may have either:
    - gmail_message_id
    - external_message_id
    - only email_subject

    The previous approach missed some email rows, causing:
    - Date not found
    - Original Email disabled
  ...
  return (
    <div className="p-10 space-y-8">
      ...
      <div className="text-sm text-gray-600">
        Showing {sortedOrders.length} grouped orders. Latest received emails are shown first.
      </div>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <form action="/api/entries/delete" method="POST">
                        <input
                          type="hidden"
                          name="entry_key"
                          value={getMessageKey(order.items[0])}
                        />
