...
async function extractStructured(text: string): Promise<ExtractedData> {
  ...
  function isPossibleSku(value: string) {
    ...
    // Reject telephone and contact-number patterns.
    if (
      /(?:^|[-_.])(TEL|TELEPHONE|PHONE|MOBILE|CELL|FAX|EXT)(?:$|[-_.])/i.test(
        candidate
      )
    ) {
      return false;
    ...
    if (/^\d{2,4}(?:[-./]\d{2,4}){2,}(?:[-./][A-Z]+)?$/i.test(candidate)) {
      return false;
    }

    if (/^\+?\d[\d()._-]{6,}$/i.test(candidate)) {
      return false;
    }

    // Reject common date formats.
    if (
      /^\d{1,4}[-/.]\d{1,2}[-/.]\d{1,4}$/i.test(candidate) ||
      /^\d{8}$/i.test(candidate)
    ) {
      return false;
    }

    // Reject IBAN-style prefixes and obvious address/postal tokens.
    if (/^[A-Z]{2}\d{2}$/.test(candidate)) return false;
    if (/^(ZIP|POSTAL|PIN|STREET|ROAD|AVENUE|SUITE|FLOOR)\d*$/i.test(candidate)) {
      return false;
    }
  ...
  /*
   * Generic compact purchase-order parser.
   *
   * Handles digitally extracted PDF rows where columns may collapse:
   *
   * 1. SKU123 - DESCRIPTION
   * UOM: YARD
   * 58$12.50$725.00
   *
   * No customer, document, SKU or PO-specific values are used here.
   */
  const compactPoPattern =
    /(?:^|\n)\s*\d+\s*[.)]\s*([A-Z][A-Z0-9/._-]{2,})\s*[-–—:]\s*([\s\S]*?)\s+(\d+(?:[.,]\d+)?)\s*([€$£₹])\s*(\d+(?:[.,]\d+)?)\s*\4\s*(\d+(?:[.,]\d+)?)(?=\s*(?:\n\s*\d+\s*[.)]|\n\s*TOTAL\b|$))/gim;

  for (const match of nonBankingText.matchAll(compactPoPattern)) {
    const sku = clean(match[1]);
    const quantity = parseLocalizedNumber(match[3]);
    const unitPrice = parseLocalizedNumber(match[5]);
    const lineTotal = parseLocalizedNumber(match[6]);
    ...
    description = description
      .replace(repeatedSkuPattern, " ")
      .replace(/\bUOM\s*:\s*[A-Z]+\b/gi, " ")
      .replace(/\bDRY\s+CLEAN\s+ONLY\b/gi, "DRY CLEAN ONLY")
      .replace(/\s+/g, " ")
      .trim();
    ...
    if (descriptionParts.length > 1) {
      ...
      for (const part of descriptionParts) {
        ...
        if (
          !uniqueParts.some(
            (existing) =>
              existing.toUpperCase() === normalizedPart ||
              existing.toUpperCase().includes(normalizedPart) ||
              normalizedPart.includes(existing.toUpperCase())
          )
        ) {
    ...
    for (const word of words) {
      const previous = deduplicatedWords[deduplicatedWords.length - 1];

      if (!previous || previous.toUpperCase() !== word.toUpperCase()) {
        deduplicatedWords.push(word);
      }
    }

    description = deduplicatedWords.join(" ").trim();
    ...
    addItem({
      ...
      line_total:
        ...
        (quantity !== null && unitPrice !== null
          ? quantity * unitPrice
          : null),
      currency: currency(match[4] || detectedCurrency),
      notes: description,
    });
  }
  ...
  for (const line of lines) {
    ...
    if (
      /\b(?:FROM|TO|CC|BCC|SUBJECT|DATE|SENT|BEGIN FORWARDED MESSAGE)\s*:/i.test(
        line
      ) ||
      /\b(?:PHONE|TELEPHONE|MOBILE|CELL|FAX|E-?MAIL|EMAIL)\b/i.test(line) ||
      /\b(?:STREET|ROAD|AVENUE|BOULEVARD|SUITE|FLOOR|LOS ANGELES|KOLKATA|INDIA|UNITED STATES)\b/i.test(
        line
      )
    ) {
      continue;
...
async function processEmail(email: EmailRow, force: boolean) {
  ...
  if (!relevance.relevant) {
    ...
    await markEmail(
      ...
      "ignored",
      "ignored",
      `Irrelevant inbound email: ${relevance.reason}`
    );

    return {
      id: email.id,
      ignored: true,
      reason: "irrelevant_inbound_email",
      relevance,
    };
  }
